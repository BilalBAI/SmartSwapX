...
contract InterestRateSwap is Ownable {
    ...
    address public partyB; // Floating rate payer

    IERC20 public token; // ERC-20 token address
    uint256 public notional; // Token A amount to be transfered to the party B in each payment

    uint256 public partyAInitMargin; // recommand: 4 * Notional
    uint256 public partyBInitMargin; // recommand: 4 * Notional
    uint256 public partyAMaintenanceMargin; // recommand: 2 * Notional
    uint256 public partyBMaintenanceMargin; // recommand: 2 * Notional
    ...
    function setSwap(
        ...
        address _token,
        uint256 _notional,
        uint256 _partyAInitMargin,
        uint256 _partyBInitMargin,
        uint256 _partyAMaintenanceMargin,
        uint256 _partyBMaintenanceMargin,
        ...
        if (
            address(token) != address(0) && tokenA.balanceOf(address(this)) > 0
        ) {
